...
# build gostint docker image and point to vagrant based Vault and MongoDB for
# image testing.  Requires `vagrant up`.  If gostint is also running in the
...
docker stop gostint || /bin/true
docker rm gostint || /bin/true

# Run gostint in foreground to allow monitoring of the log output in the
# terminal.
docker run --init -t \
  --name gostint -p 3333:3232 \
  --privileged=true \
