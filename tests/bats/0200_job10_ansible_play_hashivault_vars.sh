...
@test "Submitting job11 ansible play hashivault_vars should return json" {
  ...
  # cat ../job11_ansible_play.json | jq ".wrap_secret_id=\"$WRAPSECRETID\"" > $BATS_TMPDIR/job.json

  QNAME=$(cat ../job11_ansible_play_hashivault_vars.json | jq .qname -r)

  # encrypt job payload using vault transit secret engine
  B64=$(base64 < ../job11_ansible_play_hashivault_vars.json)
  ...
  J="$(curl -k -s https://127.0.0.1:3232/v1/api/job --header "X-Auth-Token: $TOKEN" -X POST -d @$BATS_TMPDIR/job.json | tee $BATS_TMPDIR/job11.json)"
  [ "$J" != "" ]
}

@test "job11 should be queued in the another_q queue" {

  J="$(cat $BATS_TMPDIR/job11.json)"
  echo "J=$J" >&2
  id=$(echo $J | jq ._id -r)
...
@test "Status should eventually be success" {
  ...
  echo "$R" > $BATS_TMPDIR/job11.final.json
  [ "$status" == "success" ]
}

@test "Should have final output in json" {
  R="$(cat $BATS_TMPDIR/job11.final.json)"

  echo "R:$R" >&2
  output="$(echo $R | jq .output -r)"

  echo "$output" | grep "ok=4"  # from vault secret interpolation
}
