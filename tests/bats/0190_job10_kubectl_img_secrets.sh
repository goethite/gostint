...
@test "Submitting job10 kubectl help should return json" {
  ...
  # cat ../job10_kubectl_helm.json | jq ".wrap_secret_id=\"$WRAPSECRETID\"" > $BATS_TMPDIR/job.json

  QNAME=$(cat ../job10_kubectl_img_secrets.json | jq .qname -r)

  # encrypt job payload using vault transit secret engine
  B64=$(base64 < ../job10_kubectl_img_secrets.json)
  ...
  J="$(curl -k -s https://127.0.0.1:3232/v1/api/job --header "X-Auth-Token: $TOKEN" -X POST -d @$BATS_TMPDIR/job.json | tee $BATS_TMPDIR/job10.json)"
  [ "$J" != "" ]
}

@test "job10 should be queued in the play job10 queue" {

  J="$(cat $BATS_TMPDIR/job10.json)"
  ...
  [ "$id" != "" ] && [ "$status" == "queued" ] && [ "$qname" == "play job10" ]
}
...
@test "Status should eventually be success" {
  ...
  echo "$R" > $BATS_TMPDIR/job10.final.json
  [ "$status" == "success" ]
}
