...
@test "Submitting job3 shell with secret injection should return json" {
  ...
  J="$(curl -k -s https://127.0.0.1:3232/v1/api/job --header "X-Secret-Token: $SECRETID" -X POST -d @../job3_shell_content.json | tee $BATS_TMPDIR/job3.json)"
  [ "$J" != "" ]
}

@test "job3 should be queued in the play queue" {
